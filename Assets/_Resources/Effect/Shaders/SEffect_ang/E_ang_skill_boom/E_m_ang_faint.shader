...
Shader "E_ang/E_m_ang_faint"
{
	Properties
	{
		[HideInInspector] _AlphaCutoff("Alpha Cutoff ", Range(0, 1)) = 0.5
		[HideInInspector] _EmissionColor("Emission Color", Color) = (1,1,1,1)
		[ASEBegin]_T_m_ang_star("T_m_ang_star", 2D) = "white" {}
	...
	SubShader
	{
		...
		Pass
		{
			...
			#endif
			CBUFFER_END
			sampler2D _T_m_ang_star;
			UNITY_INSTANCING_BUFFER_START(E_angE_m_ang_faint)
				UNITY_DEFINE_INSTANCED_PROP(float, _inten)
			UNITY_INSTANCING_BUFFER_END(E_angE_m_ang_faint)
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
				...
				float4 ShadowCoords = float4( 0, 0, 0, 0 );
					...
					#endif
				#endif
				float _inten_Instance = UNITY_ACCESS_INSTANCED_PROP(E_angE_m_ang_faint,_inten);
				float4 color2 = IsGammaSpace() ? float4(1,1,1,0) : float4(1,1,1,0);
				float2 texCoord4 = IN.ase_texcoord3.xy * float2( 1,1 ) + float2( 0,0 );
				float4 temp_output_5_0 = ( _inten_Instance * color2 * tex2D( _T_m_ang_star, texCoord4 ).r );
				
				float3 BakedAlbedo = 0;
				float3 BakedEmission = 0;
				float3 Color = ( IN.ase_color * temp_output_5_0 ).rgb;
				float Alpha = ( IN.ase_color.b * temp_output_5_0 ).r;
        ...
        Pass
        {
			...
			half4 frag(VertexOutput IN ) : SV_TARGET
			{
				...
				surfaceDescription.Alpha = ( IN.ase_color.b * temp_output_5_0 ).r;
...
ASEEND*/
//CHKSM=9DC44AC9D9684B1A2C98B5E7717A866C66747A9B